...
while True:
    try:
        ...
        emaLong = GetEMA(df, 180, -2)
        emaShort = GetEMA(df, 14, -2)
        ...
        if amount == 0:
            if (emaLong > emaShort) and (rsi[-3] > rsi_up) and (rsi[-4] < rsi[-3]) and (rsi[-3] > rsi[-2]) and (df['close'][-2] > emaLong):
            ...
            if (emaLong < emaShort) and (rsi[-3] < rsi_down) and (rsi[-4] > rsi[-3]) and (rsi[-3] < rsi[-2]) and (df['close'][-2] < emaLong):
                coin_price = GetCoinNowPrice(okx, Target_Coin_Ticker)
